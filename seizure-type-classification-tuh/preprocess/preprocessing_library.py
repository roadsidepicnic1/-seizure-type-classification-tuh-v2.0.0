...
class Slice:
    ...
    def apply(self, data):
        ...
        print("s", s)
        print("s type", type(s))
        print("slice(None)", slice(None))
        print("data.ndim", data.ndim)
        s[-1] = slice(self.start , self.stop)
        print("s-1", s[-1])
        #s = tuple(s)
        print("s before return", s)
        print("stype before return", type(s))
        print(" s[0]", s[0])
        print("ts[1]" ,s[1])
        print(" types[0]", type(s[0]))
        print("type s[1]", type(s[1]))
        #print("data[s]",data[s])
        #print("data[s].type", data[s].type)
        return data[tuple(s)]
    ...
    def apply(self, data):
        ...
        print(self.stop)
        print(type(self.start))
        s[-1] = slice(int(self.start), int(self.stop))
        return data[s]
